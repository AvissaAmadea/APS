...
function getKatList($conn){
    $getKatListQuery = "select id_kategori, nama_kategori from kategori";
    $resultKatList = mysqli_query($conn, $getKatListQuery);

    if (!$resultKatList){
        return array('status'=>'error','massage'=>'error fetching data list: ' . mysqli_error($conn));
    }

    $KatName = array();
    while ($rowKat = mysqli_fetch_assoc($resultKatList)){
        $KatName[] = $rowKat["nama_kategori"];
    }
    return array('status'=>'success', 'katLis'=>$KatName);
}
