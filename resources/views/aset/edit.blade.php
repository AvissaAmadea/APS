    ...
    <div class="row">
        <div class="card flex-fill border-0 p-1">
            ...
            <div class="card-body mx-2">
                <form class="form text-end" method="POST" action="{{ route('aset.update', $aset->id) }}">
                    @method('PATCH')
                    @csrf
                    <div class="form-group mb-2 row">
                        <label for="nama_aset" class="col-sm-2 col-form-label">Nama Aset</label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control @error('nama_aset') is-invalid @enderror" id="nama_aset" name="nama_aset" value="{{ $aset->nama_aset }}" required autofocus>
                        </div>
                        @error('nama_aset')
                            ...
                            <select class="form-select @error('kategori_id') is-invalid @enderror" type="select" id="kategori_id" name="kategori_id" required autofocus>
                                <option value="" selected disabled>-- Pilih Kategori --</option>
                                    @foreach ($kategoris as $kategoriData)
                                        <option value="{{ $kategoriData->id }}" {{ $aset->kategori_id == $kategoriData->id ? 'selected' : null }}>
                                            {{ $kategoriData->jenis }}
                                        </option>
                                    @endforeach
                            </select>
                            ...
                            <select class="form-select @error('dinas_id') is-invalid @enderror" type="select" id="dinas_id" name="dinas_id" required autofocus>
                                <option value="" selected disabled>-- Pilih Dinas --</option>
                                    @foreach ($dinas as $dinasData)
                                        <option value="{{ $dinasData->id }}" {{ $aset->dinas_id == $dinasData->id ? 'selected' : null }}>
                    ...
                    <div class="form-group mb-2 row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control @error('detail') is-invalid @enderror" id="detail" name="detail" value="{{ $aset->detail }}" required autofocus>
                        </div>
                        @error('detail')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                    ...
                    <div class="form-group mb-2 row">
                        ...
                        <div class="col-sm-10">
                            <select class="form-select @error('status_aset') is-invalid @enderror" type="select" id="status_aset" name="status_aset" required autofocus>
                                <option value="" selected disabled>-- Pilih Status --</option>
                                @foreach (['Tersedia', 'Tidak Tersedia'] as $statuses)
                                    <option value="{{ $statuses }}" {{ $aset->status_aset == $statuses ? 'selected' : '' }}>{{ $statuses }}</option>
                                @endforeach
                            </select>
                            @error('status_aset')
                                <span class="invalid-feedback" role="alert">
